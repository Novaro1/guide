// Main guide runtime. Exposes window.Guide for the inline editor (edit.js) to hook into.
(function () {
  ...
  function autoEmbedVideos(md) {
    ...
    return md.split(/\r?\n/).map(line => {
      const t = line.trim();
      const yt = t.match(/^(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([\w-]{6,})(?:\S*)?$/);
      if (yt) return `<div class="video-embed"><iframe src="https://www.youtube.com/embed/${yt[1]}" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`;
      const vm = t.match(/^(?:https?:\/\/)?(?:www\.)?vimeo\.com\/(\d+)(?:\S*)?$/);
      if (vm) return `<div class="video-embed"><iframe src="https://player.vimeo.com/video/${vm[1]}" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen></iframe></div>`;
      return line;
    }).join("\n");
  }
  ...
  function renderSiteChrome() {
    ...
    $("site-footer").innerHTML = s.footer ? renderMarkdown(s.footer) : "";
  }
  ...
  function maybeShowWelcome() {
    ...
    showModal({ title: w.title || "Welcome", body: w.body || "", confirmLabel: "Get started", showCancel: false });
  }

  function totalDepth() { return Math.max(3, Object.keys(state.content.nodes || {}).length); }

  function renderProgress() {
    const s = state.content.site || {};
    const p = $("progress");
    if (!s.showProgress || state.editMode) { p.classList.add("hidden"); return; }
  ...
  // ---------- node rendering (viewer) ----------
  function renderNode(nodeId, opts = {}) {
    const node = state.content.nodes[nodeId];
    if (!node) {
      $("card-content").innerHTML = `<div class="text-red-300">Missing step: <code>${nodeId}</code></div>`;
      return;
    }
    if (!opts.skipHistory) state.historyStack.push(nodeId);
    persistHistory();

    // edit-mode renderer takeover
    if (state.editMode && typeof Guide.editRenderer === "function") {
      Guide.editRenderer(node);
      document.dispatchEvent(new CustomEvent("guide:rendered", { detail: { nodeId, node, editMode: true } }));
      return;
    }
      ...
      btn.addEventListener("click", () => {
        ...
        const goNext = () => {
          if (opt.nextNodeId && state.content.nodes[opt.nextNodeId]) {
            ...
            showModal({ title: "Configuration error", body: `Option points to missing step: \`${opt.nextNodeId}\``, showCancel: false });
          }
        };
    ...
    document.dispatchEvent(new CustomEvent("guide:rendered", { detail: { nodeId, node, editMode: false } }));
  }
  ...
  function persistHistory() { try { sessionStorage.setItem(STORE_KEY, JSON.stringify(state.historyStack)); } catch {} }
  function loadHistory() {
    try { const raw = sessionStorage.getItem(STORE_KEY); if (raw) state.historyStack = JSON.parse(raw) || []; }
  ...
  window.Guide = {
    ...
    escapeHtml,
    showModal,
    editRenderer: null, // edit.js sets this
  };
